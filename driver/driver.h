...
public:
	...
	const bool Init(const BOOL PhysicalMode) {
		...
		if (this->hDriver != INVALID_HANDLE_VALUE) {
			if (this->SharedBuffer = VirtualAlloc(0, sizeof(REQUEST_DATA), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) {
				UNICODE_STRING RegPath = RTL_CONSTANT_STRING(L"\\Registry\\Machine\\SOFTWARE\\kernal");
	...
	NTSTATUS ProtectMemory(PVOID src, DWORD protect, DWORD size) {
		REQUEST_PROTECT req;
		req.ProcessId = ProcessId;
		req.Address = src;
		req.InOutProtect = &protect;
		req.Size = size;

		return SendRequest(REQUEST_TYPE::PROTECT, &req);
	}

	NTSTATUS ScanCodeCave(void* dest) {

		REQUEST_QUERY req;
		req.ProcessId = ProcessId;
		req.Dest = dest;

		return SendRequest(REQUEST_TYPE::QUERY, &req);
	}
	...
	{
		HANDLE hProcessSnap = LI_FN(CreateToolhelp32Snapshot).safe_cached()(TH32CS_SNAPPROCESS, 0);
		if (hProcessSnap == INVALID_HANDLE_VALUE)
			return 0;

		PROCESSENTRY32 pe32 = { 0 };
		pe32.dwSize = sizeof(pe32);
		BOOL bRet = LI_FN(Process32First).safe_cached()(hProcessSnap, &pe32);;
		while (bRet)
		{
			if (pe32.th32ProcessID == dwPID)
			{
				LI_FN(CloseHandle).safe_cached()(hProcessSnap);
				return pe32.cntThreads;
			}
			bRet = LI_FN(Process32Next).safe_cached()(hProcessSnap, &pe32);
		}
		return 0;
	}
	...
	const UINT GetProcessId(const char* process_name) {
		...
		HANDLE snapshot = LI_FN(CreateToolhelp32Snapshot).safe_cached()(TH32CS_SNAPPROCESS, 0);
		PROCESSENTRY32 process;
		ZeroMemory(&process, sizeof(process));
		process.dwSize = sizeof(process);
		// Walkthrough all processes.

		if (LI_FN(Process32First).safe_cached()(snapshot, &process))
		{
			do
			{
				if (LI_FN(strcmp).safe_cached()(process.szExeFile, process_name) == 0)
				{
			...
			} while (LI_FN(Process32Next).safe_cached()(snapshot, &process));
		}
		LI_FN(CloseHandle).safe_cached()(snapshot);
		return pid;
	}

	const bool Attach(const char* Processname) {
...
private:
	PVOID SharedBuffer;
	HANDLE hDriver;
	ULONG64 MAGGICCODE = 0x62a4ba0a1c0f6349ul;
...
inline Driver* driver = new Driver;
...
template <typename T>
T wpm(const uintptr_t address, T buffer)
{
